...
function ScoresPageContent() {
  ...
  const handleTradeReady = (data: TradeReadyData) => setTradeData(data);
  ...
  const handleSave = () => {
    ...
    try {
      ...
      analyses.unshift({ id: Date.now().toString(), event, type: marketType, timestamp: Date.now(), polymarketOdds });
  ...
  const topOutcome = (() => {
    ...
    if (outcomeType === 'prices') {
      ...
      if (uncertain.length > 0) {
        return uncertain.reduce((prev: any, curr: any) => Math.abs(curr.odds - 50) < Math.abs(prev.odds - 50) ? curr : prev);
      }
    }
  ...
  const riskDescription = (() => {
    if (marketType === 'categorical') {
      if (outcomeType === 'prices') return categoricalOutcomes.length + ' price levels tracked. ' + topOutcome.odds + '% chance the top level is hit.';
      if (outcomeType === 'dates') return categoricalOutcomes.length + ' dates are possible. ' + topOutcome.odds + '% chance on the leading date.';
      if (outcomeType === 'candidates') return categoricalOutcomes.length + ' candidates running. ' + topOutcome.odds + '% means ' + (100 - topOutcome.odds) + '% chance someone else wins.';
      return categoricalOutcomes.length + ' ' + unitLabel + ' competing. ' + topOutcome.odds + '% means ' + (100 - topOutcome.odds) + '% chance a different outcome wins.';
    }
    return 'Market shows ' + (polymarketOdds ?? 0) + '% probability. Review all signals before deciding.';
  })();

  const positiveSignals = marketType === 'categorical'
    ? (() => {
        const sigs: string[] = [];
        if (categoricalOutcomes.length > 0) sigs.push('Highest probability of all ' + categoricalOutcomes.length + ' ' + unitLabel);
        if ((topOutcome.weekChange || 0) > 5) sigs.push('Gaining fast - up ' + (topOutcome.weekChange || 0) + '% this week');
        else if ((topOutcome.weekChange || 0) > 0) sigs.push('Momentum is trending upward this week');
        if ((topOutcome.edge || 0) > 5) sigs.push('AI signals strongly favor this outcome');
        else if ((topOutcome.edge || 0) > 0) sigs.push('AI signals are bullish on this outcome');
        if (sigs.length === 0) sigs.push('Leads all ' + unitLabel + ' in market probability');
        return sigs;
      })()
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex items-start justify-between mb-6">
          ...
          <div className="flex gap-2 flex-wrap justify-end">
            <button onClick={() => setShowSources(!showSources)} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 rounded-lg text-sm font-semibold transition-all shrink-0">
              Tune AI sources
            </button>
            <button onClick={runAnalysis} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-lg text-sm font-semibold transition-all shrink-0">
              Refresh results
            </button>
            <button onClick={handleSave} className="px-4 py-2 bg-green-600 hover:bg-green-500 rounded-lg text-sm font-semibold transition-all shrink-0">
              Save
            </button>
          </div>
        </div>
        ...
        {showSources && (
          <div className="mb-6 border border-purple-500/30 rounded-xl p-6 bg-purple-900/10">
            <div className="flex gap-2 mb-6">
              <button onClick={() => setSourceTab('weights')} className={'px-4 py-2 rounded-lg text-sm font-semibold ' + (sourceTab === 'weights' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700')}>
                Adjust weights
              </button>
              <button onClick={() => setSourceTab('marketplace')} className={'px-4 py-2 rounded-lg text-sm font-semibold ' + (sourceTab === 'marketplace' ? 'bg-purple-600 text-white' : 'bg-gray-800 text-gray-400 hover:bg-gray-700')}>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between items-center p-3 bg-gray-800 rounded-lg">
                  ...
                  <span className={'font-bold text-sm ' + (weights.news + weights.social + weights.technical === 100 ? 'text-green-400' : 'text-red-400')}>
                    ...
                    {weights.news + weights.social + weights.technical === 100 ? ' ok' : ' error'}
                  </span>
                </div>
            ...
            {showDeep && (
              <div className="border border-gray-700 rounded-xl p-5 space-y-4">
                ...
                {[
                  ...
                  { label: 'Market probability', value: Math.round((polymarketOdds || 50) * (weights.technical / 100)), color: 'bg-blue-500', desc: 'Live odds: ' + (polymarketOdds ?? 0) + '%' },
                ].map(s => (
                  <div key={s.label}>
