  ...
  return (
    <main style={{ minHeight: "100vh", background: "#070B10", color: "#fff" }}>
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "72px 20px" }}>
        ...
        <div style={{ marginTop: 30, padding: 20, borderRadius: 18, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
          <h2 style={{ margin: 0, fontSize: 22 }}>Overall Confidence</h2>
          <div style={{ fontSize: 48, fontWeight: 700, color: "#00D4FF", marginTop: 8 }}>{overall}%</div>
        </div>
        <div style={{ marginTop: 30, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
        ...
        <div style={{ marginTop: 40, padding: 18, borderRadius: 16, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
          <h3 style={{ margin: 0, fontSize: 18 }}>Suggested Research Links</h3>
          <ul style={{ marginTop: 10, lineHeight: 1.8, paddingLeft: 20 }}>
            ...
            <li><a href={`https://polymarket.com/search?q=${q}`} target="_blank" rel="noreferrer" style={{ color: "#00D4FF" }}>Search on Polymarket</a></li>
          </ul>
        </div>
      </div>
    </main>
...
function ScoreCard({ title, value }: { title: string; value: number }) {
  return (
    <div style={{ padding: 18, borderRadius: 18, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)" }}>
      ...
      <div style={{ fontSize: 36, marginTop: 10, fontWeight: 700 }}>{value}%</div>
    </div>
  );
}
