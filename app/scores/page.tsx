...
function ScoresPageContent() {
  ...
  const RightPanel = () => (
    <div>
      <div style={{ background: S.bg3, border: '1px solid ' + S.border, borderRadius: 16, padding: 24, marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.8px', color: S.text3, marginBottom: 12 }}>AI Verdict</div>
        <div style={{ fontSize: 13, fontWeight: 600, color: S.text2, marginBottom: 6 }}>{eventTitle.slice(0,60)}{eventTitle.length > 60 ? '...' : ''}</div>
        <div style={{ fontSize: 52, fontWeight: 700, letterSpacing: -2, lineHeight: 1, marginBottom: 4, color: S.text }}>{mainOdds > 0 ? mainOdds+'%' : '--'}</div>
        <div style={{ fontSize: 13, color: S.text3, marginBottom: 16 }}>{mainOdds >= 65 ? 'Likely yes' : mainOdds >= 40 ? 'Uncertain' : 'Likely no'}</div>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 8, fontSize: 13, fontWeight: 600, marginBottom: 12, background: convBg, color: convColor, border: '1px solid ' + convBorder }}>
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: convColor, display: 'inline-block' }}></span>
          {convLabel}
        </div>
        <div style={{ fontSize: 13, color: S.text2, lineHeight: 1.6, marginBottom: 16, padding: '12px 14px', background: S.bg4, borderRadius: 10, borderLeft: '3px solid ' + S.border2 }}>
          {soWhat}
        </div>
        <div style={{ fontSize: 13, fontWeight: 600, color: S.amber }}>Suggested bet: {betAmt}</div>
      </div>

      <div style={{ background: S.bg3, border: '1px solid ' + S.border, borderRadius: 16, padding: '20px 24px', marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.8px', color: S.text3, marginBottom: 14 }}>Market vs AI</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr' }}>
          {[
            { val: mainOdds+'%', label: 'Market', color: S.text2 },
            { val: mainAI+'%', label: 'AI thinks', color: S.purple2 },
            { val: (edgeVal > 0 ? '+' : '') + edgeVal.toFixed(1) + '%', label: 'Edge', color: convColor },
          ].map((c, i) => (
            <div key={i} style={{ textAlign: 'center', padding: '12px 0', borderLeft: i > 0 ? '1px solid ' + S.border : 'none' }}>
              <div style={{ fontSize: 28, fontWeight: 700, letterSpacing: -1, lineHeight: 1.1, color: c.color }}>{c.val}</div>
              <div style={{ fontSize: 11, color: S.text3, textTransform: 'uppercase' as const, letterSpacing: '0.5px', marginTop: 4 }}>{c.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ background: S.bg3, border: '1px solid ' + S.border, borderRadius: 16, padding: '20px 24px', marginBottom: 14 }}>
        <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.8px', color: S.text3, marginBottom: 14 }}>Signal breakdown</div>
        {[
          { name: 'News sentiment', type: sigNews > 20 ? 'strong' : 'mixed' as SigType, val: '+'+sigNews+'%', color: sigNews > 20 ? S.green : S.amber },
          { name: 'Community signal', type: sigSocial > 20 ? 'strong' : 'mixed' as SigType, val: '+'+sigSocial+'%', color: sigSocial > 20 ? S.green : S.amber },
          { name: 'Market probability', type: 'priced' as SigType, val: '+'+sigMarket+'%', color: S.blue },
        ].map((s, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < 2 ? '1px solid ' + S.border : 'none' }}>
            <span style={{ fontSize: 13, fontWeight: 500, color: S.text }}>{s.name}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <SignalBadge type={s.type} />
              <span style={{ fontSize: 13, fontWeight: 600, fontFamily: 'monospace', color: s.color }}>{s.val}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: S.bg3, border: '1px solid ' + S.border, borderRadius: 14, padding: '16px 20px' }}>
        <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.8px', color: S.text3, marginBottom: 12 }}>Current configuration</div>
        {[
          { label: 'News sources', val: weights.news },
          { label: 'Social signals', val: weights.social },
          { label: 'Market probability', val: weights.technical },
        ].map((c, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, padding: '6px 0', borderBottom: i < 2 ? '1px solid ' + S.border : 'none' }}>
            <span style={{ color: S.text2 }}>{c.label}</span>
            <span style={{ fontWeight: 600, fontFamily: 'monospace' }}>{c.val}%</span>
          </div>
        ))}
      </div>
    </div>
