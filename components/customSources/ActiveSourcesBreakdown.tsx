    ...
    <div style={{ marginBottom: 32, padding: 20, borderRadius: 12, background: "rgba(147,51,234,0.08)", border: "1px solid rgba(147,51,234,0.25)" }}>
      ...
      <div style={{ display: "grid", gap: 16 }}>
        {newsSources.length > 0 && (
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#e4e4e7", marginBottom: 8 }}>News ({categoryWeights.news}% total weight):</div>
            <div style={{ paddingLeft: 16, display: "grid", gap: 6 }}>
              {newsSources.map(({ source, sourceWeight, finalWeight }) => (
                <div key={source.id} style={{ fontSize: 13, color: "#d4d4d8", display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ color: "#9ca3af" }}>({sourceWeight}% of news = {finalWeight}% total)</span>
                  {!source.isDefault && <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "rgba(147,51,234,0.2)", color: "#a78bfa", fontWeight: 600 }}>CUSTOM</span>}
                </div>
              ))}
            </div>
          </div>
        ...
        {socialSources.length > 0 && (
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#e4e4e7", marginBottom: 8 }}>Social ({categoryWeights.social}% total weight):</div>
            <div style={{ paddingLeft: 16, display: "grid", gap: 6 }}>
              {socialSources.map(({ source, sourceWeight, finalWeight }) => (
                <div key={source.id} style={{ fontSize: 13, color: "#d4d4d8", display: "flex", alignItems: "center", gap: 8 }}>
                  <span>{source.isDefault ? "Default" : "Custom"}</span>
                  <span>{source.name}</span>
                  <span style={{ color: "#9ca3af" }}>({sourceWeight}% of social = {finalWeight}% total)</span>
                  {!source.isDefault && <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "rgba(147,51,234,0.2)", color: "#a78bfa", fontWeight: 600 }}>CUSTOM</span>}
                </div>
              ))}
            </div>
        ...
        {technicalSources.length > 0 && (
          <div>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#e4e4e7", marginBottom: 8 }}>Technical ({categoryWeights.technical}% total weight):</div>
            <div style={{ paddingLeft: 16, display: "grid", gap: 6 }}>
              {technicalSources.map(({ source, sourceWeight, finalWeight }) => (
                <div key={source.id} style={{ fontSize: 13, color: "#d4d4d8", display: "flex", alignItems: "center", gap: 8 }}>
                  <span>{source.isDefault ? "Default" : "Custom"}</span>
                  <span>{source.name}</span>
                  <span style={{ color: "#9ca3af" }}>({sourceWeight}% of technical = {finalWeight}% total)</span>
                  {!source.isDefault && <span style={{ fontSize: 10, padding: "2px 6px", borderRadius: 4, background: "rgba(147,51,234,0.2)", color: "#a78bfa", fontWeight: 600 }}>CUSTOM</span>}
                </div>
              ))}
            </div>
